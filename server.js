...
const HOSTINGER_URL = process.env.HOSTINGER_URL || 'https://neonultimate.com';
const SYNC_SECRET = 'neon_ultimate_secret_key';

let serverData = { leaderboard: { singleplayer: [], multiplayer: [] }, usernames: [] };

async function loadData() {
    try {
        const response = await fetch(`${HOSTINGER_URL}?secret=${SYNC_SECRET}`);
        if (response.ok) {
            const d = await response.json();
            serverData.leaderboard = {
                singleplayer: d.leaderboard?.singleplayer || [],
                multiplayer: d.leaderboard?.multiplayer || []
            };
            serverData.usernames = d.usernames || [];
            console.log('[DATA] Loaded successfully from Hostinger!');
            return;
        } else {
            console.log('[DATA] Hostinger load failed, falling back to local disk.');
        }
    } catch (e) {
        console.log('[DATA] Hostinger fetch error, reading local data: ' + e.message);
    }
    ...
    try {
        if (fs.existsSync(DATA_FILE)) {
            ...
            serverData.usernames = d.usernames || [];
            console.log('[DATA] Loaded successfully from local disk.');
        }
...
async function saveData() {
    try {
        ...
        // Fire-and-forget sync to hostinger
        fetch(HOSTINGER_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ secret: SYNC_SECRET, data: serverData })
        }).then(r => {
            if (!r.ok) console.error('[DATA] Hostinger sync failed. HTTP:', r.status);
        }).catch(err => {
            console.error('[DATA] Hostinger sync error:', err.message);
        });
    } catch (e) {
...
app.get('/api/username/check', (req, res) => {
    ...
    const taken = serverData.usernames.some(u => {
        if (typeof u === 'string') return u.toLowerCase() === name.toLowerCase();
        return u.username.toLowerCase() === name.toLowerCase();
    });
...
app.post('/api/username/register', (req, res) => {
    ...
    const existingIdx = serverData.usernames.findIndex(u => {
        if (typeof u === 'string') return u.toLowerCase() === username.toLowerCase();
        return u.username.toLowerCase() === username.toLowerCase();
    });

    if (existingIdx >= 0) {
        const existing = serverData.usernames[existingIdx];
        if (typeof existing === 'string') {
            // Claim legacy account with this new PIN
            serverData.usernames[existingIdx] = { username: existing, pin: cleanPin, profile: null };
            saveData();
            return res.json({ success: true, message: 'Legacy account claimed' });
        } else {
            if (existing.pin === cleanPin) {
                return res.json({ success: true, message: 'Logged in successfully', profile: existing.profile });
            } else {
                return res.status(401).json({ error: 'Incorrect PIN for this callsign' });
            }
        }
    }

    serverData.usernames.push({ username, pin: cleanPin, profile: null });
    saveData();
    res.json({ success: true });
});

app.post('/api/profile/update', (req, res) => {
    const { username, pin, profile } = req.body;
    if (!username || !pin || !profile) return res.status(400).json({ error: 'Missing data' });

    const u = serverData.usernames.find(u =>
        typeof u !== 'string' &&
    ...
    res.status(401).json({ error: 'Unauthorized to update profile' });
});
...
io.on('connection', (socket) => {
    ...
    function leaveRoom() {
        ...
        if (room.players.length === 0) {
            ...
            // During a live game, check if all remaining players are now dead
    ...
    // ---- Force-start (host only, seed lobby, ≥2 players) ----
    socket.on('force_start', () => {
        const room = roomForSocket(socket.id);
        if (!room || room.started || room.public) return;
        // Only the room creator (first player) may force-start with ≥2 players
        if (room.players[0]?.socketId === socket.id && room.players.length >= 2) {
            startRoom(room);
        }
    });
    ...
    // ---- Disconnect with 10-second forfeit grace for active games ----
    socket.on('disconnect', () => {
        console.log(`[-] ${socket.id}`);
        const room = roomForSocket(socket.id);
        if (room && room.started) {
            // Notify room of impending forfeit
            socket.to(room.id).emit('player_disconnecting', { socketId: socket.id, graceMs: 10000 });
            // Mark player as disconnected so game-over check skips them for now
            const p = room.players.find(pl => pl.socketId === socket.id);
            if (p) p._disconnecting = true;
            setTimeout(() => {
                // If they haven't reconnected (still in room), forfeit them
                const stillInRoom = roomForSocket(socket.id);
                if (stillInRoom) {
                    stillInRoom.players = stillInRoom.players.filter(pl => pl.socketId !== socket.id);
                    io.to(stillInRoom.id).emit('player_left', { socketId: socket.id });
                    const allDead = stillInRoom.players.every(pl => !pl.alive);
                    if (allDead || stillInRoom.players.length === 0) {
                        const results = [...stillInRoom.players]
                            .sort((a, b) => b.score - a.score)
                            .map((pl, i) => ({ socketId: pl.socketId, username: pl.username, score: pl.score, rank: i + 1 }));
                        io.to(stillInRoom.id).emit('room_game_over', { results });
                        rooms.delete(stillInRoom.id);
                    }
                }
            }, 10000);
        } else {
